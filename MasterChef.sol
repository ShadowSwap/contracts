...
contract MasterChef is Ownable {
    ...
    // The EGG TOKEN!
    ShadeToken public shade;
    // Dev address.
    address public devaddr;
    // EGG tokens created per block.
    uint256 public shadePerBlock;
    // Bonus muliplier for early shade makers.
    ...
    constructor(
        ...
        address _devaddr,
        address _feeAddress,
        uint256 _shadePerBlock,
        uint256 _startBlock
    ) public {
        shade = _shade;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        shadePerBlock = _shadePerBlock;
    ...
    function pendingEgg(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 shadeReward = multiplier.mul(shadePerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accEggPerShare = accEggPerShare.add(shadeReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 shadeReward = multiplier.mul(shadePerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        shade.mint(devaddr, shadeReward.div(10));
        shade.mint(address(this), shadeReward);
        pool.accEggPerShare = pool.accEggPerShare.add(shadeReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    // Safe shade transfer function, just in case if rounding error causes pool to not have enough EGGs.
    function safeEggTransfer(address _to, uint256 _amount) internal {
        uint256 shadeBal = shade.balanceOf(address(this));
        if (_amount > shadeBal) {
            shade.transfer(_to, shadeBal);
        } else {
            shade.transfer(_to, _amount);
        }
    }
    ...
    function updateEmissionRate(uint256 _shadePerBlock) public onlyOwner {
        massUpdatePools();
        shadePerBlock = _shadePerBlock;
    }
}
